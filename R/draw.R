...
setMethod(
  "draw", signature(x = "data.frame", setup = "SampleSetup"), 
  function(x, setup, i = 1) drawS3(x, setup, i))

setMethod(
  "draw", signature(x = "data.frame", setup = "VirtualSampleControl"), 
  function(x, setup) {
    setK(setup, 1)
...
setMethod(
  "draw", signature(x = "data.frame", setup = "character"), 
  function(x, setup, ...) {
    if(length(setup) != 1) {
    ...
    if(isTRUE(setup == "TwoStageSampleControl")) {
      setup <- TwoStageSampleControl(...)
    } else setup <- new(setup, ...)
    draw(x, setup)
  })

setMethod(
  "draw", signature(x = "data.frame", setup = "missing"), 
  function(x, setup, ...) draw(x, SampleControl(...)))
